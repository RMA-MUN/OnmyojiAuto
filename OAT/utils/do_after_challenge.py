...
def close_game(window_hwnd: int) -> bool:
    ...
    try:
        if window_hwnd:
            ...
            logger.info(f"已发送关闭命令到窗口：{window_title} (句柄: {window_hwnd})")
            ...
            if confirm_hwnd:
                ...
                logger.info(f"检测到确认弹窗：{confirm_window_title} (句柄: {confirm_hwnd})")
                logger.info(f"发送确认关闭命令到窗口：{confirm_title}")
                ...
                try:
                    ...
                    logger.error(f"激活确认弹窗时发生错误：{str(e)}")
                ...
                for attempt in range(retry_count):
                    logger.info(f"第{attempt+1}/{retry_count}次尝试确认关闭")
                    ...
                    try:
                        ...
                        if not win32gui.IsWindow(confirm_hwnd):
                            logger.info("确认弹窗已关闭，操作成功")
                            success = True
                            break
                            
                    except Exception as e:
                        logger.error(f"方式1（完整回车键）失败：{str(e)}")

                    
                    # 方式2：尝试查找确认按钮并点击
                    try:
                        logger.info("尝试查找并点击确认按钮")
                        ...
                        if confirm_btn_hwnd:
                            logger.info(f"找到确认按钮，句柄：{confirm_btn_hwnd}")
                            # 发送点击消息
                        ...
                        logger.error(f"方式2（点击确认按钮）失败：{str(e)}")
        ...
        logger.error(f"关闭游戏窗口时发生错误：{str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def close_program() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"当前活动线程数: {len(threads)}")
            ...
            logger.error(f"终止线程时发生错误：{str(thread_e)}")
            # 如果还是不行，就直接退出
            sys.exit(0)
    except Exception as e:
